...
def read_stat_plots(reads,minqual,minLength,threads):
        ...
        os.system('%s -t %s --raw --o %s.stats_plots --minqual %s --minlength %s --fastq %s' % (nanoplot,threads,reads,minqual,minLength,reads))
...
def filter_trim_reads(reads,filtered_reads,minqual,minLength,threads):
        ...
        os.system('%s -l %s -q %s %s > %s' % (nanofilt,minLength,minqual,reads,filtered_reads))
...
def assembly_flye(longReads_filtered,genome_size,threads):
	...
	os.system('%s --nano-raw %s -g %s -o %s.flye -t %s' % (flye,longReads_filtered,genome_size,longReads_filtered,threads))
...
def assembly_miniasm(longReads_filtered,threads):
	...
	os.system('%s -x ava-ont -t12 %s %s | gzip -1 > %s.miniasm.paf.gz' % (minimap2,longReads_filtered,longReads_filtered,longReads_filtered))
	...
	os.system('%s -a -t 12 %s.miniasm.gfa.fasta %s > %s.miniasm.sam' % (minimap2,longReads_filtered,longReads_filtered,longReads_filtered))
	# polishing with Racon
	os.system('%s -t 12 %s %s.miniasm.sam %s.miniasm.gfa.fasta > %s.miniasm.racon' % (racon,longReads_filtered,longReads_filtered,longReads_filtered,longReads_filtered))
...
def predict_genes(assembly,threads):
        ...
        os.system('%s --cpus %s --outdir %s.prokka %s' % (prokka,threads,assembly,assembly))
